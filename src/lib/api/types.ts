...
export interface CoachingSession {
  ...
  expert_id: string
  title: string
  /** Session start time. Empty string for `awaiting_schedule` rows. */
  session_date: string
  duration: number
  /**
   * `awaiting_schedule` is a pre-allocated session (e.g. program quota
   * coach calls) that the member hasn't booked yet — `session_date` is
   ...
   * (e.g. coach calendar disconnected).
   */
